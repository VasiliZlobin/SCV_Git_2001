...
2. Для отправки данных на удаленный репозиторий используется команда `git push`. Git выдаст подсказку при необходимости использования ее параметров. Например:
```
```